...
def source_surface_from_request(req) -> str | None:
    """
    Analytics v1 is web-only.

    Web Core marks its domain calls with X-Source-Surface: web.
    Telegram/bot/system callers should not get Users analytics in this milestone.
    """
...
def emit_users_event(
    ...
    *,
    ...
    """
    Best-effort server-side Analytics emission from Users.
    ...
    if not source_surface:
        logging.info("analytics_users_event_skipped_non_web event=%s", event_name)
        return False
    ...
    if not base_url or not function_key:
        logging.warning(
            "analytics_users_not_configured event=%s base_configured=%s key_configured=%s",
            event_name,
            bool(base_url),
            bool(function_key),
        )
    ...
    if require_user_id and not cleaned_user_id:
        logging.warning(
            "analytics_users_event_skipped_missing_user_id event=%s subject_type=%s subject_id=%s",
            event_name,
            subject_type,
            subject_id,
        )
    ...
    try:
        with request.urlopen(analytics_req, timeout=timeout) as response:
            ...
            if status >= 400:
                logging.warning(
                    "analytics_users_emit_failed event=%s status=%s subject_type=%s subject_id=%s",
                    event_name,
                    status,
                    subject_type,
                    subject_id,
        ...
        logging.warning(
            "analytics_users_emit_http_failed event=%s status=%s error=%s subject_type=%s subject_id=%s",
            event_name,
            exc.code,
            error_code,
            subject_type,
        ...
        logging.warning(
            "analytics_users_emit_request_failed event=%s error_type=%s subject_type=%s subject_id=%s",
            event_name,
            type(exc).__name__,
            subject_type,
            subject_id,
        ...
        logging.warning(
            "analytics_users_emit_unexpected_failed event=%s error_type=%s subject_type=%s subject_id=%s",
            event_name,
            type(exc).__name__,
            subject_type,
            subject_id,

...
def test_update_user_cv(base_url, auth_headers, default_user, shared_state):
    ...
    print("CV update response:", response.status_code, response.text)

    assert response.status_code == 200
    body = response.json()

    assert body["message"].lower().startswith("cv updated")

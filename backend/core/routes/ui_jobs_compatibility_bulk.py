...
def create_blueprint(auth):
    bp = Blueprint("ui_jobs_compatibility_bulk", __name__)

    @bp.route("/ui/jobs/compatibility", methods=["POST"])
    @auth.login_required
    def ui_jobs_compatibility_bulk(*, context):
        ...
        r = fx_post_json(
            f"{jobs_base()}/jobs/compatibility",
            headers=headers,

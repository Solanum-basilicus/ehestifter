...
function normalizeRemoteType(value) {
  ...
  if (normalized === 'onsite' || normalized === 'inoffice') {
    return 'On-site';
  }
  ...
  return [
    {
      ...
      countryCode: null,
      cityName:
        typeof postal.addressLocality === 'string'
          ? postal.addressLocality.trim() || null
          : null,
      region:
        typeof postal.addressRegion === 'string'
...
async function fetchGreenhouseDetails(candidate, context) {
  ...
  const endpoint = new URL(
    `https://${GREENHOUSE_HOST}/v1/boards/${encodeURIComponent(tenant)}/jobs/${encodeURIComponent(externalId)}`,
  );
  ...
  return {
    ...
    applyUrl:
      typeof payload.absolute_url === 'string'
...
async function fetchAshbyDetails(candidate, context) {
  ...
  if (!job) {
    throw new Error(
      `Ashby job ${externalId} was not present on board ${tenant}`,
    );
  }
  ...
  return {
    description:
      plain || htmlToPlainText(job.descriptionHtml),
...
async function fetchDetails(candidate, context) {
  ...
  switch (provider) {
    case 'greenhouse':
      return {
        supported: true,
        ...(await fetchGreenhouseDetails(candidate, context)),
      };

    case 'ashby':
      return {
        supported: true,
        ...(await fetchAshbyDetails(candidate, context)),
      };
...
export async function enrichCandidateDetails(
          ...
          return {
            ...
            candidate: {
              ...
              detail: {
                ...
                provider:
                  candidate.canonicalIdentity?.provider ?? null,
              },
            },
          };
        }
  ...
  for (const item of fetched) {
    output[item.index] = item.candidate;
  }
